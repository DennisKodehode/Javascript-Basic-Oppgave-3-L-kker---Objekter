...
// utility function to get random city so the loop isnt as messy
...
for (let i = 0; i < people.length; i++) {
  if (people[i].name === "Otto") continue;
    ...
    people[i].city = getRandomValue(cities);
...
******************************************************************************/
function diceRoller(rolls, faces) {
  let diceRoll = [];
  for (let i = 0; i < rolls; i++) {
    diceRoll.push(Math.ceil(Math.random() * faces));
  }
  return diceRoll;
}
console.log(diceRoller(20, 4));
